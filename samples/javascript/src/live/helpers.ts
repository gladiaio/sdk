...
import type { Recorder, StreamingAudioFormat } from './types.ts'
...
export function printMessage(message: LiveV2WebSocketMessage, startTime: number) {
  if (message.type === 'transcript') {
    const { text, start, end, language } = message.data.utterance
    console.log(
      `${(Date.now() - startTime).toString().padStart(6, ' ')} | ${formatSeconds(start)} --> ${formatSeconds(
        end
      )} | ${language} | ${message.data.is_final ? 'F' : 'P'} | ${text.trim()}`
    )
...
function parseAudioFile(
  ...
  const sample_rate = buffer.readUInt32LE(24) as StreamingAudioFormat['sample_rate']
  const bit_depth = buffer.readUInt16LE(34) as StreamingAudioFormat['bit_depth']
...
export function getAudioFileFormat(filename: string): StreamingAudioFormat {
  const { startDataChunk, buffer, ...format } = parseAudioFile(filename)
  return format
}
...
export function initFileRecorder(
  ...
  const { startDataChunk, buffer, bit_depth, sample_rate, channels } = parseAudioFile(filename)
